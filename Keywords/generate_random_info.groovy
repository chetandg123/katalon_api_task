...
class generate_random_info {
	...
	def generateRandomFirstName() {
		...
		// Generate random first name composed of syllables
		def numSyllables = random.nextInt(2) + 1 // Randomly choose 1 or 2 syllables
		for (int i = 0; i < numSyllables; i++) {
			def syllable = ""
			// First letter is a consonant
			syllable += consonants[random.nextInt(consonants.length())].toUpperCase()
			// Randomly alternate between vowels and consonants
			for (int j = 0; j < random.nextInt(3) + 1; j++) {
				syllable += vowels[random.nextInt(vowels.length())]
				syllable += consonants[random.nextInt(consonants.length())]
			}
			firstNames.add(syllable)
		}
	...
	def generateRandomLastName() {
		...
		// Generate random last name composed of syllables
		def numSyllables = random.nextInt(2) + 1 // Randomly choose 1 or 2 syllables
		for (int i = 0; i < numSyllables; i++) {
			def syllable = ""
			// First letter is a consonant
